...
{
	if (((unsigned)lpProcName > 0xFFFF) && (!lstrcmpA(lpProcName, "VirtualAlloc2")))
	{	
		PPEB Peb;
		Peb = NtCurrentPeb();
		...
		if(AshPerformChromiumDetectionFromModuleExports(Peb->ImageBaseAddress) == STATUS_SUCCESS) {
	...
	IN	PCWSTR	ModuleName)
{
	...
	//KexLogDetailEvent(L"GetModuleHandleW: %s", ModuleName);
