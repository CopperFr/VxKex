	...
	OUT	PPWSTR	ThreadDescription)
{
	...
	*ThreadDescription = NULL;
	...
	if (NT_SUCCESS(Status)) {
		*ThreadDescription = (PWSTR)LocalAlloc(LMEM_FIXED, Description->Length + sizeof(WCHAR));
		if (!*ThreadDescription) {
			Status = STATUS_NO_MEMORY;
		} else {
			RtlMoveMemory(*ThreadDescription, Description->Buffer, Description->Length);
			*ThreadDescription[Description->Length / sizeof(WCHAR)] = L'\0';
		}
	...
	ULONG		Flags)
{
	KexLogInformationEvent(
		L"GetSystemCpuSetInformation called\r\n\r\n"
		L"Buffer length: %d",
		BufferLength
	);
	
	if (BufferLength == 0 && ReturnedLength != NULL) {
		*ReturnedLength = 0;
		return TRUE;
	}
	...
	if (priority == 4) {
		unless(KexData->IfeoParameters.DisableAppSpecific) {
			if (KexData->Flags & KEXDATA_FLAG_CHROMIUM) {
				KexLogWarningEvent(L"GetThreadPriority returned: %d", priority);
				priority = -4;
			}
		}
	}
