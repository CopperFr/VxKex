...
PWSTR CDECL _Wcsftime(
	VOID)
{
	NTSTATUS Status;
	PWSTR UcrtcsftimeResult;
	STATIC P_W_GETTNAMES UcrtWcsftime = NULL;

	if (!UcrtWcsftime) {
		Status = GetUcrtProcAddress("_Wcsftime", (PPVOID)&UcrtWcsftime);
		ASSERT(NT_SUCCESS(Status));
	...
	ASSUME(UcrtWcsftime != NULL);

	UcrtcsftimeResult = UcrtWcsftime();
	ASSERT(UcrtcsftimeResult != NULL);

	if (!UcrtcsftimeResult) {
		return NULL;
	}

	Status = TransferToMsvcrtHeap(
		(PPVOID)&UcrtcsftimeResult,
		(wcslen(UcrtcsftimeResult) + 1) * sizeof(WCHAR));
