	...
	IN	LPARAM	LParam)
{
	...
	if (Message == WM_INITDIALOG) {
		...
		if (PropSheetData) {
			...
			WCHAR ExplorerExe[MAX_PATH];
			
			KxCfgSetConfiguration(PropSheetData->ExeFullPath, &ProgramConfiguration, NULL);
			PropSheetData->SettingsChanged = FALSE;

			GetWindowsDirectory(WinDir, ARRAYSIZE(WinDir));
			PathCchCombine(ExplorerExe, MAX_PATH, WinDir, L"explorer.exe");
			
			if (StringEqualIW(ExplorerExe, PropSheetData->ExeFullPath)) {
				INT UserResponse;
				...
				UserResponse = MessageBoxF(
					...
					L"Do you want to restart explorer now?");
				...
				if (UserResponse == IDYES) {
					...
					Explorer = OpenProcess(PROCESS_TERMINATE, FALSE, Pid);
					TerminateProcess(Explorer, 2);
