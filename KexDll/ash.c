	...
	VOID)
{
	...
	Peb = NtCurrentPeb();
	// Windows EXE
	if (RtlPrefixUnicodeString(&KexData->WinDir, &Peb->ProcessParameters->ImagePathName, TRUE)) {
		// Window Explorer
		if (AshExeBaseNameIs(L"explorer.exe")) {
			return TRUE;
		}
		// Command Prompt
		if (AshExeBaseNameIs(L"cmd.exe")) {
			return TRUE;
		}
	}
	// Visual Studio
	if (AshExeBaseNameIs(L"devenv.exe")) {		
		return TRUE;
	}
	// WinDbg
	if (AshExeBaseNameIs(L"windbg.exe")) {
		return TRUE;
	}
	// Microsoft Visual Studio Remote Debugging Monitor
	if (AshExeBaseNameIs(L"msvsmon.exe")) {
		return TRUE;
	}
	return FALSE;
}