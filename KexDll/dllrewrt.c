	...
	OUT	PUNICODE_STRING			RewrittenDllName)
{
	...
	if (RtlPrefixUnicodeString(&ApiPrefix, &CleanDllName, TRUE) ||
		...
		UNICODE_STRING ApiMsWinCore7;
		RtlInitConstantUnicodeString(&ApiMsWinCore7, L"api-ms-win-core-string-l1-1-0");
		if (KexRtlFindUnicodeSubstring(&ApiMsWinCore7, &CleanDllName, TRUE))
		{
			// Don't rewrite this
		} 
		else
