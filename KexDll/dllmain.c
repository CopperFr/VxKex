	...
	IN	PRTL_VERIFIER_PROVIDER_DESCRIPTOR	*Descriptor)
{
	...
	if (Reason == DLL_PROCESS_VERIFIER) {
		...
		// For Windows Exe minimal functionality
		...
		unless (KexData->IfeoParameters.DisableAppSpecific) {
			...
			// APPSPECIFICHACK: Zoner Photo Studio X doesn't work if 
			// D3D11On12CreateDevice is present because it need IDXGIFactory4.
			if (AshExeBaseNameIs(L"zps.exe")) {
				UNICODE_STRING D3D11;
				RtlInitConstantUnicodeString(&D3D11, L"d3d11");
				Status = KexRemoveDllRewriteEntry(&D3D11);
			}
