...
KEXAPI NTSTATUS NTAPI KexRtlGetPersistedStateLocation(
	...
	IN  PWCHAR              TargetPath,
	IN  ULONG               BufferLengthIn,
	OUT PULONG              BufferLengthOut OPTIONAL)
{
	return STATUS_NOT_IMPLEMENTED;
}
...
KEXAPI VOID NTAPI KexRtlGetDeviceFamilyInfoEnum(
	OUT PULONGLONG	PullUAPInfo,
	OUT PDWORD		PulDeviceFamily,
	OUT PDWORD		PulDeviceForm)
{
	if (PullUAPInfo) *PullUAPInfo = 0;
	if (PulDeviceFamily) *PulDeviceFamily = DEVICEFAMILYINFOENUM_DESKTOP;
	if (PulDeviceForm) *PulDeviceForm = DEVICEFAMILYDEVICEFORM_DESKTOP;
}

KEXAPI DWORD NTAPI KexRtlConvertDeviceFamilyInfoToString(
	IN OUT	PDWORD PulDeviceFamilyBufferSize,
	IN OUT	PDWORD PulDeviceFormBufferSize,
	OUT		PWSTR  DeviceFamily,
	OUT		PWSTR  DeviceForm)
{
	if (PulDeviceFamilyBufferSize && !DeviceFamily) *PulDeviceFamilyBufferSize = 16;
	if (PulDeviceFormBufferSize && !DeviceForm) *PulDeviceFormBufferSize = 16;
	if (DeviceFamily) StringCchCopyW(DeviceFamily, *PulDeviceFamilyBufferSize, L"Desktop");
	if (DeviceForm) StringCchCopyW(DeviceForm, *PulDeviceFormBufferSize, L"Desktop");
	return S_OK;
}
