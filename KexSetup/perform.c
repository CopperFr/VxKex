	...
	VOID)
{
	...
	// remove old PDBs
	KexSetupFormatPath(TargetPath, L"%s\\32\\*.pdb", KexDir); //Kex32
	KexSetupDeleteFilesBySpec(TargetPath);

	KexSetupFormatPath(TargetPath, L"%s\\32", KexDir);  //Kex32
	...
	if (Is64BitOS) {
		// remove old PDBs
		KexSetupFormatPath(TargetPath, L"%s\\64\\*.pdb", KexDir);  //Kex64
		KexSetupDeleteFilesBySpec(TargetPath);

		KexSetupFormatPath(TargetPath, L"%s\\64", KexDir); //Kex64
